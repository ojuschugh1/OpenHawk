...
/// Launch the HawkEye TUI. Blocks until the user quits.
pub fn run() -> Result<()> {
    enable_raw_mode()?;
...
mod tests {
    ...
    fn switch_panel_alerts_to_dashboard() {
        ...
        app.apply_action(AppAction::SwitchPanel);
        assert_eq!(app.view, HawkEyeView::OrchestrationGraph);
    }

    #[test]
    fn switch_panel_orchestration_to_dashboard() {
        let mut app = HawkEyeApp::new();
        app.view = HawkEyeView::OrchestrationGraph;
        app.apply_action(AppAction::SwitchPanel);
        assert_eq!(app.view, HawkEyeView::Dashboard);
    }
