...
fn minimal_manifest(entry: &str) -> AgentManifest {
    AgentManifest {
        ...
        resources: Resources { cpu_percent: 25, memory_mb: 512, max_open_fds: 64 },
...
async fn test_spawn_creates_child_and_records_in_db() {
    ...
    let pid = manager.spawn(minimal_manifest("sleep 60")).await.expect("spawn should succeed");
    assert!(pid > 0);
...
async fn test_stop_fast_process_completes() {
    let manager = make_manager();
    let pid = manager.spawn(minimal_manifest("echo done")).await.expect("spawn should succeed");
...
fn test_permission_fs_allow() {
    ...
    assert!(matches!(guard.check_fs_access(1, Path::new("/tmp/file.txt")), PermissionResult::Allowed));
}
...
fn test_permission_fs_deny() {
    let mut guard = make_guard();
    guard.register(1, minimal_manifest("echo"));
    assert!(matches!(guard.check_fs_access(1, Path::new("/etc/shadow")), PermissionResult::Denied { .. }));
}
...
fn test_permission_network_allow() {
    let mut guard = make_guard();
    guard.register(1, minimal_manifest("echo"));
    assert!(matches!(guard.check_network(1, "https://api.openai.com/v1/completions"), PermissionResult::Allowed));
}
...
fn test_permission_network_deny() {
    let mut guard = make_guard();
    guard.register(1, minimal_manifest("echo"));
    assert!(matches!(guard.check_network(1, "https://malicious.example.com"), PermissionResult::Denied { .. }));
}
...
fn test_permission_command_allow() {
    let mut guard = make_guard();
    guard.register(1, minimal_manifest("echo"));
    assert!(matches!(guard.check_command(1, "echo hello world"), PermissionResult::Allowed));
}
...
fn test_permission_secret_allow() {
    let mut guard = make_guard();
    guard.register(1, minimal_manifest("echo"));
    assert!(matches!(guard.check_secret(1, "MY_KEY"), PermissionResult::Allowed));
}
...
fn test_permission_secret_deny() {
    let mut guard = make_guard();
    guard.register(1, minimal_manifest("echo"));
    assert!(matches!(guard.check_secret(1, "OTHER_KEY"), PermissionResult::Denied { .. }));
}
...
fn test_trust_bypasses_fs_check() {
    ...
    assert!(matches!(guard.check_fs_access(1, Path::new("/etc/shadow")), PermissionResult::Allowed));
}
...
fn test_trust_bypasses_network_check() {
    ...
    guard.register(1, minimal_manifest("echo"));
    guard.trust("test-agent", "session-1".to_string());
    assert!(matches!(guard.check_network(1, "https://any.endpoint.com"), PermissionResult::Allowed));
}
...
fn test_trust_bypasses_secret_check() {
    ...
    guard.register(1, minimal_manifest("echo"));
    guard.trust("test-agent", "session-1".to_string());
    assert!(matches!(guard.check_secret(1, "UNKNOWN_SECRET"), PermissionResult::Allowed));
}
...
async fn test_resource_monitor_memory_exceeded_event() {
    let monitor = ResourceMonitor::new();
    monitor.register(42, ResourceLimits { cpu_percent: 10, memory_mb: 512, max_open_fds: 64 });
